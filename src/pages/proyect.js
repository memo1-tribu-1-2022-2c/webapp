...
function Proyect() {
  ...
  const handleEditProyect = () => {
    navigate(`/proyectos/proyectsList/${id}/editProyect`, {
      state: { project: project, tasks: tasks },
    });
  };

  const handleCreateTask = () => {
    navigate(`/proyectos/proyectsList/${id}/createTask`, {
      state: { id: id, tasks: tasks },
    });
  };
  ...
  const getProjectInfo = async () => {
    ...
    const response = await fetch(
      `https://squad2-2022-2c.herokuapp.com/api/v1/projects/${id}`,
      requestOptions
    );
  ...
  const getTasks = async () => {
    ...
    const response = await fetch(
      `https://squad2-2022-2c.herokuapp.com/api/v1/projects/${id}/tasks`,
      requestOptions
    );
    const responseData = await response.json();
        ...
        <Box py="2">
              ...
              <HStack spacing="20" justifyContent="space-between">
                ...
                <VStack>
                  ...
                  <Text>
                    Horas estimadas:{" "}
                    {
                      project.estimatedHours /*  && project.estimatedHours.split("T")[0] */
                    }
                  </Text>
                  <Text>
                    Horas tabajadas:{" "}
                    {
                      project.workedHours /* && project.estimatedHours.split("T")[0] */
                    }
                  </Text>
                </VStack>
                <VStack>
                  ...
                  <Text>
                    Iniciado{" "}
                    {project.startingDate && project.startingDate.split("T")[0]}
                  </Text>
                  <Text>
                    Finalización{" "}
                    {project.endingDate && project.endingDate.split("T")[0]}
                  </Text>
                </VStack>
              </HStack>
            ) : (
          ...
          >
            <Input
              ...
              onChange={(value) => {
                filterTasks(value.target.value);
              }}
            />
            <Button
              borderRadius={"5"}
