    ...
    <Modal isOpen={props.isOpen} onClose={close}>
      ...
      <ModalContent bg="gray.300">
        <ModalHeader>
          {!newTicket &&
            !viewTickets &&
            `Soporte producto ${props.product_name} (${props.version.number}) para ${props.razon_social}`}
          {newTicket &&
            `Nuevo ticket para ${props.razon_social} en la version ${props.version.number} del producto ${props.product_name}`}
          {viewTickets &&
            `Tickets de ${props.razon_social} para la version ${props.version.number} del producto ${props.product_name}`}
        </ModalHeader>
          ...
          <>
            ...
            <ModalFooter justifyContent="space-between">
              ...
              <Button onClick={() => setViewTickets(!viewTickets)}>
                {viewTickets ? "Volver" : "Ver tickets"}
              </Button>
              <Button onClick={close} colorScheme={newTicket ? "red" : null}>
                {"Cerrar"}
              </Button>
            </ModalFooter>
          </>
