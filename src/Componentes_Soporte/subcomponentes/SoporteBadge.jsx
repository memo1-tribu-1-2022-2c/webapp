  ...
  const find_support_status = async () => {
    try {
      const response = await (
        await axios.get(
          `https://squad2-2022-2c.herokuapp.com/api/v1/projects/client/${props.client_id}`
        )
      ).data;
      const has_support = response.filter((value) => {
        return (
          value.versionId == props.version.version_id &&
          value.projectType == "SOPORTE"
        );
      });
      setLoading(false);
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <Td>
            <Tag colorScheme={hasSupport ? "green" : "red"}>
              {hasSupport ? "Con soporte" : "Sin soporte"}
            </Tag>
          </Td>
          <Td>
            <Button
              onClick={onOpen}
              size="sm"
              colorScheme={hasSupport ? "green" : "blue"}
            >
              {hasSupport ? "Tickets" : "Alta soporte"}
            </Button>
          </Td>
        </>
      )}
