    ...
    if (!check_result) {
      try {
        ...
        const response = await (
          await axios.post("https://modulo-soporte.onrender.com/product", data)
        ).data;
        props.loadNewProduct(response);
        setErrorText(
          `Producto creado exitosamente con id: ${response.product_id}`
        );
        setDone(true);
      } catch {
        setError(true);
        ...
        <ModalContent bg="gray.300">
          ...
          <ModalBody>
            <Text
              ...
              marginBottom="2.5%"
              fontSize="18px"
              color={error ? "red.600" : "blackAlpha.900"}
            >
              {errorText}
            </Text>
            {!done ? (
              <FormControl>
                <FormLabel color={errorProduct ? "red.600" : "blackAlpha.900"}>
                  {errorProduct ? errorTextProduct : "Nombre del producto"}
                </FormLabel>
                <Input
                  bg="white"
                  type="text"
                ...
                <FormLabel
                  marginTop="5%"
                  color={errorVersion ? "red.600" : "blackAlpha.900"}
                >
                  {errorVersion ? errorTextVersion : "Nombre de la version"}
                </FormLabel>
                <Input
                  bg="white"
                  type="text"
                ...
                <FormHelperText marginBottom="5%">
                  El producto debe tener al menos una version
                </FormHelperText>
              </FormControl>
            ) : null}
          </ModalBody>
